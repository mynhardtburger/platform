...
import { DEFAULT_MODEL_ID } from "@/services/api/models";
...
export default function CreateScheduledSessionPage() {
  ...
  const handleWorkflowChange = (value: string) => {
    ...
    if (workflow) {
      setWorkflowSelection({ gitUrl: workflow.gitUrl, branch: workflow.branch, path: workflow.path });
    }
  };
    ...
    createMutation.mutate(
      {
        ...
        data: {
          ...
          sessionTemplate: {
            ...
            ...(workflowSelection ? { activeWorkflow: workflowSelection } : {}),
              ...
              {repos.map((repo, index) => (
                <div key={index} className="rounded-md border p-4 space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <Button type="button" variant="ghost" size="sm" onClick={() => removeRepo(index)} disabled={createMutation.isPending}>
